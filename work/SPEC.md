...
> Four-surface framing (map / drawer / list / swipe stack) and the v1 → v2 → v3 progression are documented in `context/UX_STRATEGY.md`. End-goal architecture (five-agent topology, two merchant surfaces, the wedge of generative offers within merchant-set bounds) is captured in `context/END_GOAL_ARCHITECTURE.md`. The demo cut below is the v1 framing — "city wallet that surfaces offers when the moment is right" — bridged to v2 by the cross-merchant swipe stack already shipped. **The definitive recording flow + merchant pitch (one opinionated synthesis across all the above) lives in `context/USER_FLOW_AND_MERCHANT_DASHBOARD.md` — read that first if you're recording.**
...
- stack: consumer = React Native + Expo + TypeScript (native dev client, local `s()` style tokens) recorded on iOS Simulator; merchant inbox = small static React (Vite) web app; FastAPI backend; SQLite. rules out: Next.js phone-frame mock; Flutter; tRPC; Postgres. revisit if: native simulator flakes — fall back to a prebuilt recording or web phone-frame only as last resort.
- llm: Azure OpenAI (`gpt-5.5` via the `rapidata-hackathon-resource` deployment) accessed through Pydantic AI structured agents (`MOMENTMARKT_LLM_PROVIDER=azure`). rules out: direct OpenAI/Anthropic billing; live on-device SLM in demo; LiteLLM gateway (dropped — not in deps). revisit if: Azure endpoint flakes — swap providers via the same Pydantic AI dispatcher (`MOMENTMARKT_LLM_PROVIDER=openrouter` or unset string-form for `openai:*`).
...
- Production native iOS/Android release pipelines (the demo uses a local Expo dev client build only).
...
- The LLM decision was edited in-place above to match the as-built backend (Pydantic AI + Azure; LiteLLM dropped from deps). Locking this and any other post-build drifts together is a candidate `spec-v05` refinement — process step the team owns; do not bump unilaterally.
